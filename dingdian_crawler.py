...
class dingdian_crawler(object):
	...
	def get_novel_content(self,start,num=0):
		...
		if start+int(num)>len(self.catalog_key):
			print 'maybe out of article list range'
			print 'please reset start or nun'
			return
	...
	def get_article_url(self,index):
		try:
			print self.catalog[self.catalog_key[index-1]]
			return self.catalog[self.catalog_key[index-1]]
		except Exception:
			print 'out of article list range'
			print 'please reset index'
	...
	def download_html(self,url,times=5):
		try:
			...
			if times:
				print 'download fail'
				print 'try download again:',(6-times)
				times = times-1
				time.sleep(2)
